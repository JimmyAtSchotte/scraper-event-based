...
public class HtmlPage : WebEntity
{
    ...
    private static readonly Regex JavascriptRegex =
        new(@"<script\b[^>]*\bsrc=[""']([^""']+)[""'][^>]*>", RegexOptions.IgnoreCase);

    private static readonly Regex LinksRegex = new(@"<a\b[^>]*\bhref=[""']([^""']+)[""'][^>]*>",
    RegexOptions.IgnoreCase);

    private static readonly Regex ImagesRegex =
        new(@"<img\b[^>]*\bsrc=[""']([^""']+)[""'][^>]*>", RegexOptions.IgnoreCase);

    private static readonly Regex CssRegex = new(@"<link\b[^>]*\bhref=[""']([^""']+)[""'][^>]*>",
    RegexOptions.IgnoreCase);

    public HtmlPage(byte[] bytes, Uri uri) : base(bytes,
    uri.AbsolutePath.EndsWith("/") ? new Uri(uri, "index.html") : uri) { }
    ...
    public override IEnumerable<Uri> ListLinkedResources()
    {
        ...
        resources.AddRange(JavascriptRegex.Matches(htmlNoComments).Where(x => x.Success)
